...
Инициализация хранилища React-обёртки осуществляется через функцию `createReactStore`, абсолютно аналогично тому, как происходит инициализация через `createObservableStore`. Оба подхода используют схожую сигнатуру.
