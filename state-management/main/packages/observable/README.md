...
#### 🖼️ Механизм Snapshot перед/после обновления

При каждом вызове `update()`:

- ✅ создаётся _снимок_ текущего состояния (без глубокого копирования)
- 🧠 старое значение сохраняется в историю (при включенной `customLimitsHistory`)
- 🔁 запускаются middleware (если есть)
- ✍️ выполняется обновление значения
- 🔍 генерируется новый снимок и сравниваются хэши
- 📣 подписчики уведомляются **только если реально изменилось значение**
