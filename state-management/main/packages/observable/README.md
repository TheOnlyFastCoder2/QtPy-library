...
`Accessor<R>` — это **магическая вспомогательная функция**, которая позволяет безопасно обращаться к значениям в состоянии `store`, особенно когда путь к данным содержит **динамическую часть** (например, индекс массива или ID).
...
- А теперь представь, что `2` — это переменная `index`, которая может меняться:
  ...
  ```ts
  const accessor = (t) => store.$.items[t(index)];
  ```
...
- Это делается через **анализ тела функции и регулярки**:
  ...
  - Внутри `t(index)` → `5`
  - `store.$.items[t(index)]` превращается в `"items.5"`
...
Метод store.get(pathOrAccessor) используется для безопасного получения значения из стора — по строковому пути ("user.age") или Accessor-функции. В отличие от прямого доступа через store.$.myObject, он гарантирует корректное извлечение даже при работе с реактивными объектами, проксями и вложенными структурами. Это особенно важно,`чтобы избежать ошибок вроде Uncaught TypeError: Illegal invocation`, которые могут возникнуть при вызове нативных методов DOM или работы с объектами вне контекста. Метод также улучшает читаемость, типизацию и делает код предсказуемым при масштабировании.
...
Это основной метод для синхронного обновления значения в сторе. Он принимает путь в виде строки или Accessor-функции, а также новое значение или функцию с предыдущим значением. Обеспечивает типобезопасность, поддерживает "тихое" обновление через keepQuiet, записывает историю для undo/redo, активирует подписки только по затронутому пути и идеально подходит для точечного контроля состояния.
...
3. **Функцией** `(cur) => next`: вычисляет новое значение на основе текущего:
...
- **Примеры:**
  ...
  store.subscribeToPath("items", () => {
    // Каждый раз, когда список items изменится, выводим его текущее состояние
    console.log("items", store.$.items);
  });
  store.update("items", (prevStore) => {
    // вызываем: добавляем два элемента в конец массива
    prevStore.push(21);
    prevStore.push(2323);
    return prevStore;
  });
  store.update("items", (prevStore) => {
    // не вызываем: функция возвращает тот же массив без изменений
    return prevStore;
  });

  store.update("items", (prevStore) => {
    // не вызываем: снова возвращаем массив без изменений
    return prevStore;
  });

  store.update("items", (prevStore) => {
    // вызываем: разворачиваем порядок элементов в массиве
    return prevStore.reverse();
  });

  store.update("items", (prevStore) => {
    // вызываем: ещё раз добавляем два элемента
    prevStore.push(21);
    prevStore.push(2323);
    return prevStore;
  });
